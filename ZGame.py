...
def show_shop_screen(screen) -> Optional[str]:
    ...
    try:
        ...
        catalog = shop_support.build_shop_catalog(_THIS_MODULE)
    finally:
        # just a guard; we clear this flag once we leave the function
        globals().pop("_in_shop_ui", None)
    locked_ids = shop_support.load_locked_ids(_THIS_MODULE)

    def _persist_locked_ids():
        shop_support.persist_locked_ids(_THIS_MODULE, locked_ids)

    _persist_locked_ids()

    def _prop_level(it):
        return shop_support.prop_level(_THIS_MODULE, it)

    def _owned_live_text(it, lvl: int | None):
        return shop_support.owned_live_text(_THIS_MODULE, it, lvl)

    def _prop_max_level(it):
        return shop_support.prop_max_level(it)

    def _prop_at_cap(it):
        return shop_support.prop_at_cap(_THIS_MODULE, it)

    def roll_offers():
        return shop_support.roll_offers(_THIS_MODULE, catalog, locked_ids)
