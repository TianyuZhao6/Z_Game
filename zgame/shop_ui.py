...
async def show_shop_screen_web(game, screen) -> Optional[str]:
    """Async web shop screen with cooperative yields for browser runtimes."""
    runtime = _runtime(game)
    meta = _meta(game)
    if not runtime.get('_resume_shop_cache', False):
        game._clear_shop_cache()
    ...
    def _prop_at_cap(it):
        return shop_support.prop_at_cap(it)
    ...
    while True:
        ...
        owned_rows = []
        if owned:
            title = font.render('Owned', True, game.UI_TEXT)
            screen.blit(title, (left_margin, y_offset - 38))
            for ent in owned:
                itm = ent['itm']
                lvl = ent['lvl']
                max_lvl = ent['max']
                lvl_str = f"{lvl}/{max_lvl}" if max_lvl else f"x{lvl}"
                text = f"{itm['name']}: {lvl_str}"
                surf = font.render(text, True, game.UI_TEXT)
                rect = surf.get_rect(topleft=(left_margin, y_offset))
                screen.blit(surf, rect)
                owned_rows.append((rect, ent))
                y_offset += 26
        btn_w, btn_h = 200, 56
        close = pygame.Rect(0, 0, btn_w, btn_h)
        close.center = (game.VIEW_W // 2, game.VIEW_H - 80)
        pygame.draw.rect(screen, (60, 60, 64), close, border_radius=10)
        pygame.draw.rect(screen, (100, 110, 130), close, 2, border_radius=10)
        close_txt = btn_font.render('NEXT', True, (235, 235, 235))
        screen.blit(close_txt, close_txt.get_rect(center=close.center))
        ...
        for ev in pygame.event.get():
            ...
            game._sync_web_input_event(ev)
            if ev.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            if is_web_interaction_event(ev):
                game._resume_bgm_if_needed(min_interval_s=0.0)
            if ev.type == pygame.KEYDOWN and ev.key == pygame.K_ESCAPE:
                bg = screen.copy()
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                if close.collidepoint(ev.pos):
                    ...
                    if loan_outcome:
                        game.show_shady_loan_popup(screen, loan_outcome)
                    chosen_biome = await game.show_biome_picker_in_shop_web(screen)
                    if chosen_biome in ('__HOME__', '__RESTART__', '__EXIT__'):
                        if chosen_biome == '__RESTART__':
                            runtime['_restart_from_shop'] = True
                        return {'__HOME__': 'home', '__RESTART__': 'restart', '__EXIT__': 'exit'}[chosen_biome]
...
async def show_biome_picker_in_shop_web(game, screen) -> str:
    """Async web biome picker to avoid blocking the browser loop."""
    title_font = pygame.font.SysFont(None, 56)
    font = pygame.font.SysFont(None, 26)
    back_font = pygame.font.SysFont(None, 48)
    names = list(game.SCENE_BIOMES)
