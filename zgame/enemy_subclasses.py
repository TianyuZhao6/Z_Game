...
def install(game):
    ...
    class MemoryDevourerBoss(game.Enemy):
        ...
        def __init__(self, grid_pos: tuple[int, int], level_idx: int):
            ...
            self.skill_t = 0.0
            super().__init__((gx, gy), attack=int(game.MEMDEV_CONTACT_DAMAGE), speed=int(max(1, game.MEMDEV_SPEED)), ztype='boss_mem', hp=boss_hp)
    ...
    class MistClone(game.Enemy):
        ...
        def __init__(self, gx: int, gy: int):
            super().__init__((gx, gy), attack=8, speed=int(game.MIST_SPEED * game.CELL_SIZE / game.CELL_SIZE), ztype='mist_clone', hp=1)
        ...
        def update_special(self, dt, player, enemies, enemy_shots, game_state=None):
            if self.hp <= 0 and (not getattr(self, '_mist_boom', False)):
                game_state.spawn_acid_pool(self.rect.centerx, self.rect.centery, r=int(game.CELL_SIZE * 0.6), life=1.2, dps=8, slow_frac=0.25)
    ...
    class MistweaverBoss(game.Enemy):
        ...
        def __init__(self, grid_pos: tuple[int, int], level_idx: int):
            gx, gy = grid_pos
            super().__init__((gx, gy), attack=game.MIST_CONTACT_DAMAGE, speed=int(game.MIST_SPEED), ztype='boss_mist', hp=int(game.MIST_BASE_HP * (1 + 0.12 * max(0, level_idx - 9))))
            self.is_boss = True
        ...
        def _do_blink(self, game_state):
            ...
            game_state.spawn_acid_pool(cx, cy, r=int(game.CELL_SIZE * 0.9), life=game.MIST_DOOR_STAY, dps=game.MIST_DOOR_DPS, slow_frac=game.MIST_DOOR_SLOW, style='mist_door')
            game_state.spawn_acid_pool(tx, ty, r=int(game.CELL_SIZE * 0.9), life=game.MIST_DOOR_STAY, dps=game.MIST_DOOR_DPS, slow_frac=game.MIST_DOOR_SLOW, style='mist_door')
                ...
                if self._blade_cd <= 0:
                    ...
                    for i in range(-1, 2):
                        ...
                        for k in range(1, 5):
                            ...
                            game_state.spawn_acid_pool(x, y, r=int(game.CELL_SIZE * 0.45), life=game.MIST_P1_STRIP_TIME, dps=game.MIST_P1_STRIP_DPS, slow_frac=game.MIST_P1_STRIP_SLOW, style='mist')
            ...
            if self.phase == 2:
                ...
                if self._storm_cd <= 0:
                    ...
                    for _ in range(game.MIST_P2_STORM_POINTS):
                        ...
                        pts.append((x, y))
                    for x, y in pts:
                        game_state.spawn_telegraph(self.rect.centerx, self.rect.centery, r=22, life=game.MIST_P2_STORM_WIND, kind='dash_mist', payload={'points': [(x, y)], 'radius': int(game.CELL_SIZE * 0.5), 'life': 4.0, 'dps': game.MIST_P2_POOL_DPS, 'slow': game.MIST_P2_POOL_SLOW}, color=game.HAZARD_STYLES['mist']['ring'])
                ...
                if random.random() < 0.007:
                    ...
                    game_state.spawn_acid_pool(rx, ry, r=game.MIST_SILENCE_RADIUS, life=game.MIST_SILENCE_TIME, dps=0, slow_frac=0.5, style='mist')
            if self.phase == 3:
                next_pct = getattr(self, '_sonar_next', 0.7)
                while hp_pct <= next_pct and next_pct >= 0.0:
                    game_state.spawn_telegraph(self.rect.centerx, self.rect.centery, r=int(self.radius * 1.8), life=0.6, kind='dash_mist', payload={'note': 'mist_sonar'}, color=game.HAZARD_STYLES['mist']['ring'])
            ...
            if self._ring_bursts_left > 0:
                ...
                if self._ring_burst_t <= 0.0:
                    for i in range(game.MIST_RING_PROJECTILES):
                        ...
                        enemy_shots.append(game.MistShot(self.rect.centerx, self.rect.centery, vx, vy, game.MIST_RING_DAMAGE, radius=10, color=game.HAZARD_STYLES['mist']['ring']))
                    self._ring_bursts_left -= 1
                    self._ring_burst_t = 0.2
                    game_state.spawn_telegraph(self.rect.centerx, self.rect.centery, r=int(self.radius * 0.95), life=0.2, kind='acid', color=game.HAZARD_STYLES['mist']['ring'])
    ...
    game.__dict__.update({'MemoryDevourerBoss': MemoryDevourerBoss, 'MistClone': MistClone, 'MistweaverBoss': MistweaverBoss})
