...
def main_game_loop(config, enemy_cards_collected):
    ...
    enemies = [Enemy(pos, attack=10, speed=ENEMY_SPEED) for pos in enemy_starts]
    running = True
    game_result = None
...
if __name__ == "__main__":
    ...
    while True:
        ...
        result, reward = main_game_loop(config, enemy_cards_collected)
        if result == "next":
            current_level += 1
            if reward:
